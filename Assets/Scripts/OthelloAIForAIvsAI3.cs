...
using System.IO;
 ...
 public class OthelloAIForAIvsAI3 : MonoBehaviour
 {
     ...
     void Start() {
         ...
         CntGames = 0;
         File.AppendAllText("/Users/oogamitakuya/Documents/Othello/time_minmax.txt","-1"+System.Environment.NewLine);
         File.AppendAllText("/Users/oogamitakuya/Documents/Othello/time_ab.txt","-1"+System.Environment.NewLine);
     ...
     void AIPlay() {
        ...
        if(board.NowTurn == Board.WhiteTurn) {
            ...
            sw.Start();
            put = GetAIPutFromBoardAB(board,AIInformation);
            sw.Stop();
            File.AppendAllText("/Users/oogamitakuya/Documents/Othello/time_ab.txt",sw.ElapsedMilliseconds.ToString()+System.Environment.NewLine);
        }
        else {
            var sw = new System.Diagnostics.Stopwatch();
            sw.Start();
            ...
            sw.Stop();
            File.AppendAllText("/Users/oogamitakuya/Documents/Othello/time_minmax.txt",sw.ElapsedMilliseconds.ToString()+System.Environment.NewLine);
    ...
    double EvaluationValueAB(Board boardToEvaluate, int nowDepth,double alpha,double beta) {
        double res;
        const double INF = 1e9;
        double tmp_value;
        Board newBoard;
        ...
        if(boardToEvaluate.NowTurn == board.NowTurn) {
            ...
            foreach(ulong put in puts) {
                newBoard = new Board(boardToEvaluate);
                newBoard.UpdateBoard(put);
                tmp_value = EvaluationValueAB(newBoard,nowDepth + 1, alpha,beta);
                if (tmp_value > res) {
                    res = tmp_value;
                    alpha = res;
                }
     ...
     ulong GetAIPutFromBoardAB(Board board, PlayerInformation AIInformation) {
         const double INF = 1e9;
         List<ulong> puts = board.MakePlayerLegalPutList();
         AIInformation.LastNumberOfHands.Add(puts.Count);
         ...
         for (int i = 0; i < puts.Count; ++i) {
             Board newBoard = new Board(board);
             newBoard.UpdateBoard(puts[i]);
             evals[i] = EvaluationValueAB(newBoard, 1,-INF,INF);
             if (evals[i] > bestEval) bestEval = evals[i]; 
         }
         AIInformation.UpdateLastAvgNumberOfHands();
         List<ulong> bestPuts = new List<ulong>();
