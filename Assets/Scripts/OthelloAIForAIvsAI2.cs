...
using System.Text;
//to generate opening books

public class OthelloAIForAIvsAI2 : MonoBehaviour
{
    ...
    const int maxGames = 484492;
    public int CntGames{ get; set; } 
    public double bestEval;

    public int DictFinished = 0;

    public int PlayedHands = 0;

    StreamReader sr = new StreamReader("/Users/oogamitakuya/Documents/Othello/record.out");
    Dictionary<System.Tuple<ulong,ulong>, Dictionary<ulong,int>> OpeningsBlack = new Dictionary<System.Tuple<ulong,ulong>, Dictionary<ulong,int>>();
    Dictionary<System.Tuple<ulong,ulong>, Dictionary<ulong,int>> OpeningsWhite = new Dictionary<System.Tuple<ulong,ulong>, Dictionary<ulong,int>>();

    Dictionary<System.Tuple<ulong,ulong>,ulong> WhitePuts = new Dictionary<System.Tuple<ulong,ulong>, ulong>();
    Dictionary<System.Tuple<ulong,ulong>,ulong> BlackPuts = new Dictionary<System.Tuple<ulong,ulong>, ulong>(); 

    void Start() {
        Random.InitState(System.DateTime.Now.Millisecond);
        //var OpeningsWhite = new Dictionary<Tuple<ulong,ulong>, Dictionary<ulong,int>>();
        //var OpeningsBlack = new Dictionary<Tuple<ulong,ulong>, Dictionary<ulong,int>>();
        ...
        CntGames = 0;
        //StreamReader sr = new StreamReader("/Users/oogamitakuya/Documents/Othello/record.out", Encoding.GetEncoding("us-ascii"));
    ...
    void Update() {
        if(board.GameFinished) {
            if (++CntGames < maxGames){
                //board.GetResult2();
                PlayedHands = 0;
                Debug.Log("finished"+CntGames);
                RestartButtonCilcked();
            }
            else {
                if (DictFinished ==0) {
                    foreach (var opening in OpeningsWhite) {
                        int guard = -1;
                        ulong selected_put = 0x0000000000000000;
                        foreach (var put_cand in opening.Value) {
                            if (put_cand.Value > guard) {
                                guard = put_cand.Value;
                                selected_put = put_cand.Key;
                            }
                        }
                        WhitePuts.Add(opening.Key,selected_put);
                    }
                    foreach (var opening in OpeningsBlack) {
                        int guard = -1;
                        ulong selected_put = 0x0000000000000000;
                        foreach (var put_cand in opening.Value) {
                            if (put_cand.Value > guard) {
                        ...
                        BlackPuts.Add(opening.Key,selected_put);
                    }
                    foreach (var whiteput in WhitePuts) {
                        File.AppendAllText("/Users/oogamitakuya/Documents/Othello/whitedict","W.Add(Tuple.Create((ulong)0x"+whiteput.Key.Item1.ToString("x")+",(ulong)0x"+whiteput.Key.Item2.ToString("x")+"),(ulong)0x"+whiteput.Value.ToString("x")+");"+System.Environment.NewLine);
                    }
                    foreach (var blackput in BlackPuts) {
                        File.AppendAllText("/Users/oogamitakuya/Documents/Othello/blackdict","B.Add(Tuple.Create((ulong)0x"+blackput.Key.Item1.ToString("x")+",(ulong)0x"+blackput.Key.Item2.ToString("x")+"),(ulong)0x"+blackput.Value.ToString("x")+");"+System.Environment.NewLine);
                        //File.AppendAllText("/Users/oogamitakuya/Documents/Othello/blackdict",blackput.Key.Item1.ToString("x")+" "+blackput.Key.Item2.ToString("x")+" "+blackput.Value.ToString("x")+System.Environment.NewLine);
                    }   
                    DictFinished = 1;  
                }     
            } 
    ...
    void AIPlay() {
        ...
        if (PlayedHands < 8) {
            if (board.NowTurn == Board.WhiteTurn) {
                ...
                blackBoard = tmp;    
                var nkey = System.Tuple.Create(whiteBoard,blackBoard);
                if (OpeningsWhite.ContainsKey(nkey)) {
                    if (OpeningsWhite[nkey].ContainsKey(put)) {
                        OpeningsWhite[nkey][put] += 1;
                    }
                    else {
                        OpeningsWhite[nkey].Add(put,1);
                    }
                }
                else {
                    OpeningsWhite.Add(nkey,new Dictionary<ulong,int>());
                    OpeningsWhite[nkey].Add(put,1);
                }  
            }
            else {
                var nkey = System.Tuple.Create(blackBoard,whiteBoard);
                if (OpeningsBlack.ContainsKey(nkey)) {
                    if (OpeningsBlack[nkey].ContainsKey(put)) {
                        OpeningsBlack[nkey][put] += 1;
                    }
                    else {
                        OpeningsBlack[nkey].Add(put,1);
                    }
                }
                else {
                    OpeningsBlack.Add(nkey,new Dictionary<ulong,int>());
                    OpeningsBlack[nkey].Add(put,1);
                }
            }
        }
    ...
    ulong IOToPut(string IO) {
        ...
        int y = (int)System.Char.GetNumericValue(IO[1]);
