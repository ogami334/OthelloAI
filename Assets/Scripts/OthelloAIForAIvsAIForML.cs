...
using System.IO;
 ...
 public class OthelloAIForAIvsAIForML : MonoBehaviour
 {
    ...
    int EvaluationValue_fullsearch_ab(Board boardToEvaluate,int rest_depth,int alpha,int beta) {
        int res;
        int INF = 80;
        int tmp_value;
        Board newBoard;
        ...
        // 再帰の終了条件: 読みの深さが上限に達した場合
        if(rest_depth==0) {
            res = board.BitCount(boardToEvaluate.PlayerBoard) - board.BitCount(boardToEvaluate.OpponentBoard);//added
        ...
        if(boardToEvaluate.NowTurn == board.NowTurn) {
            ...
            foreach(ulong put in puts) {
                newBoard = new Board(boardToEvaluate);
                newBoard.UpdateBoard(put);
                tmp_value = EvaluationValue_fullsearch_ab(newBoard,rest_depth-1,alpha,beta);
                if (tmp_value > res) {
                    res = tmp_value;
                    alpha = res;
                }
     ...
     ulong GetAIPutFromBoard(Board board, PlayerInformation AIInformation) {
         ...
         int num_of_stone = board.BitCount(board.PlayerBoard) + board.BitCount(board.OpponentBoard);
         if (num_of_stone >= 52) {
            ...
            for (int i = 0; i < puts.Count; ++i) {
                ...
                evals[i] = EvaluationValue_fullsearch_ab(newBoard, 64-num_of_stone-1,-80,80);
                if (newBoard.NowTurn!=board.NowTurn) {
                    File.AppendAllText(@"C:\Users\denjo\Downloads\12ゲームAI リバーシ\Othello\MLData\data"+(num_of_stone+1).ToString()+".txt",newBoard.PlayerBoard.ToString()+" "+ newBoard.OpponentBoard.ToString()+" "+(-evals[i]).ToString()+System.Environment.NewLine);
                    File.AppendAllText(@"C:\Users\denjo\Downloads\12ゲームAI リバーシ\Othello\MLData\Alldata.txt",(num_of_stone+1).ToString() + newBoard.PlayerBoard.ToString()+" "+ newBoard.OpponentBoard.ToString()+" "+(-evals[i]).ToString()+System.Environment.NewLine);
                }
                else {
                    File.AppendAllText(@"C:\Users\denjo\Downloads\12ゲームAI リバーシ\Othello\MLData\data"+(num_of_stone+1).ToString()+".txt",newBoard.PlayerBoard.ToString()+" "+ newBoard.OpponentBoard.ToString()+" "+(evals[i]).ToString()+System.Environment.NewLine);
                    File.AppendAllText(@"C:\Users\denjo\Downloads\12ゲームAI リバーシ\Othello\MLData\Alldata.txt",(num_of_stone+1).ToString() + newBoard.PlayerBoard.ToString()+" "+ newBoard.OpponentBoard.ToString()+" "+(evals[i]).ToString()+System.Environment.NewLine);                    
            ...
            for (int i = 0; i < puts.Count; ++i) {
                ...
                if (evals[i] > bestEval) bestEval = evals[i];
                //eval of newBoard ,newBoard.playerBoard,newBoard.OpponentBoard
            }
            AIInformation.UpdateLastAvgNumberOfHands();
            List<ulong> bestPuts = new List<ulong>();
            for (int i = 0; i < puts.Count; ++i) {
